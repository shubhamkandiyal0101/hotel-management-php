<?php
   ...
   // signup hotel user
	...
	function hotelAllRoomsDetails($hotelId) {
		...
		if($roomsFilterData != NULL) {
			while($row = $roomsFilterData->fetch_assoc() ){
				...
				// ends here ~ query for get data
				if(mysqli_num_rows($filterRoomBooking) != 0) {
					$row['room_booked_status'] = 1;
				}
	...
	function bookHotelRoom() {
		...
		$query = "SELECT id FROM room_booking_details WHERE ((check_in_date >='".$check_in_final_date."' AND check_in_date >='".$check_out_final_date."') OR (check_out_date >='".$check_in_final_date."' AND check_out_date >='".$check_out_final_date."') OR (check_in_date <='".$check_in_final_date."' AND check_out_date >='".$check_out_final_date."')) AND hotel_room_id='".$hotel_room_id."'";
		$isRoomBooked = $GLOBALS['dbconnection']->query($query);
	...
	function getBookedHotelRoom() {
		...
		$query = "SELECT * FROM room_booking_details WHERE reserved_user_id='".$hotelUserId."'";
		...
		if(mysqli_num_rows($filterBookedHotelRooms) == 0) {
			...
			while($row = $filterBookedHotelRooms->fetch_assoc() ){
				$currentRoomId = $row['hotel_room_id'];
				// query for get booked hotel details from room-booking_details table
				$query = "SELECT * FROM hotel_rooms WHERE id='".$currentRoomId."'";
				$filterRoomBooking = $GLOBALS['dbconnection']->query($query);
				...
				$row['room_number'] = $filterRoomBooking['room_number'];
				$row['room_price'] = $filterRoomBooking['room_price'];
				$row['room_type'] = $filterRoomBooking['room_type'];
	...
	// edit hotel registration
	function editHotelRegistration() {
		$target_dir = $_SERVER['DOCUMENT_ROOT']."/uploadImg/";
		$formData = json_decode($_POST['otherInfo']);
		...
		print_r($_FILES);
			if(count($_FILES) != 0) {
				...
				print_r($target_file);
				if(move_uploaded_file($_FILES["file"]["tmp_name"], $target_file) == true) {
					$query = "UPDATE hotels SET hotel_name='".$hotelName."', hotel_addr='". $hotelAddr."', hotel_city='". $hotelCity."', hotel_district='". $hotelDistrict."', hotel_pincode='". $hotelPin."', hotel_state='".$hotelState."', hotel_desc='". $hotelDescription."', hotel_image='". $imgName."' WHERE id='". $hotelId."'";
					$serverResp = $GLOBALS['dbconnection']->query($query);

					// send response back to AJAX according to $serverResp
					if($serverResp == true) {
						$respArray = array("status" => 200,"messageType" => "success", "data" => "null","responseType" => "success","message" => "Hotel Details Updated Successfully");
						echo json_encode($respArray);
					} else {
						$respArray = array("status" => 200,"messageType" => "error", "data" => "null","responseType" => "success","message" => "Error while updating Hotel Details");
						echo json_encode($respArray);
					}
					// ends here ~ send response back to AJAX according to $serverResp
				}
			} else {
				$query = "UPDATE hotels SET hotel_name='".$hotelName."', hotel_addr='". $hotelAddr."', hotel_city='". $hotelCity."', hotel_district='". $hotelDistrict."', hotel_pincode='". $hotelPin."', hotel_state='".$hotelState."', hotel_desc='". $hotelDescription."' WHERE id='". $hotelId."'";
				$serverResp = $GLOBALS['dbconnection']->query($query);
	...
	// ends here ~ edit hotel registration
